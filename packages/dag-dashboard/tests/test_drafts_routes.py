...
def test_list_drafts_sorted_newest_first(client, workflows_dir):
    ...
    create_draft(workflows_dir, "test-workflow", "20260101T120000Z", "content: old")
    create_draft(workflows_dir, "test-workflow", "20260102T120000Z", "content: middle")
    create_draft(workflows_dir, "test-workflow", "20260103T120000Z", "content: new")
    
    response = client.get("/api/workflows/test-workflow/drafts")
    assert response.status_code == 200
    items = response.json()
    assert len(items) == 3
    assert items[0]["timestamp"] == "20260103T120000Z"
    assert items[1]["timestamp"] == "20260102T120000Z"
    assert items[2]["timestamp"] == "20260101T120000Z"
...
def test_get_draft_returns_content(client, workflows_dir):
    ...
    create_draft(workflows_dir, "test-workflow", "20260101T120000Z", content)
    
    response = client.get("/api/workflows/test-workflow/drafts/20260101T120000Z")
    assert response.status_code == 200
    data = response.json()
    assert data["timestamp"] == "20260101T120000Z"
...
def test_create_draft_prunes_to_50(client, workflows_dir):
    ...
    # Verify oldest deleted (20260100T120000Z)
    oldest_path = drafts_dir / "20260100T120000Z.yaml"
...
def test_update_draft_inplace(client, workflows_dir):
    ...
    create_draft(workflows_dir, "test-workflow", "20260101T120000Z", "old content")
    
    new_content = "new content"
    response = client.put(
        "/api/workflows/test-workflow/drafts/20260101T120000Z",
        json={"content": new_content}
    )
    assert response.status_code == 204
    
    # Verify content changed
    draft_path = workflows_dir / ".drafts" / "test-workflow" / "20260101T120000Z.yaml"
...
def test_delete_draft_204(client, workflows_dir):
    ...
    response = client.delete("/api/workflows/test-workflow/drafts/20260101T120000Z")
    assert response.status_code == 204
...
"""
    ...
    response = client.post("/api/workflows/test-workflow/drafts/20260101T120000Z/publish")
    assert response.status_code == 200
    data = response.json()
    ...
    assert data["source_timestamp"] == "20260101T120000Z"
...
def test_publish_validates_schema_before_rename(client, workflows_dir):
    ...
    create_draft(workflows_dir, "test-workflow", "20260101T120000Z", invalid_content)
...
def test_publish_invalid_yaml(client, workflows_dir):
    ...
    invalid_yaml = "{\ninvalid yaml: [\n"
    create_draft(workflows_dir, "test-workflow", "20260101T120000Z", invalid_yaml)
...
def test_multi_dir_collision_first_wins(tmp_path):
    ...
    create_draft(dir_a, "test-workflow", "20260101T120000Z", "content from A")
    create_draft(dir_b, "test-workflow", "20260101T120000Z", "content from B")
...
def test_rejects_name_with_dotdot(client):
    ...
    # Note: FastAPI routing rejects this before our handler (404 is acceptable)
    response = client.get("/api/workflows/../etc/drafts")
...
def test_rejects_invalid_timestamp_traversal(client):
    ...
    # Note: This gets through routing but fails our timestamp validation
    response = client.get("/api/workflows/test-workflow/drafts/../../etc")
    assert response.status_code in [400, 404]
