...
from promptc import ParseError
...
class TestParseErrors:
    ...
    def test_mismatched_closing_tag(self) -> None:
        ...
        # Should report mismatched closing tag
        assert "Mismatched closing tag" in exc_info.value.message
        assert "tag1" in exc_info.value.message
        assert "tag2" in exc_info.value.message
