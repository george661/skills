...
from promptc import LimitExceededError, ParserConfig
...
class TestLimits:
    ...
    def test_tag_count_limit_enforced(self) -> None:
        config = ParserConfig(max_tags_per_file=5)
    ...
    def test_node_count_limit_enforced(self) -> None:
        config = ParserConfig(max_ast_nodes=5)
        parser = Parser(config)
    ...
    def test_configurable_limits(self) -> None:
        config = ParserConfig(max_tags_per_file=100, max_ast_nodes=200)
        parser = Parser(config)
    ...
    def test_no_stack_overflow_on_deep_nesting(self) -> None:
        ...
        parser = Parser(ParserConfig(max_tags_per_file=1000, max_ast_nodes=2000))
